...
$(function (){
	...
	$("[name=type_id], [name=ap_layout]").on("change", function(){
		if($(this).val()==50 || $(this).val()==52 || $(this).val()==53){
			$("[name=ap_layout]").removeAttr('required').parent().hide();
		...
		$.post("?task=var&action=street_check", "street="+$("#str").val(), function(html){
			if(parseInt(html) == 0 && parseInt($('[name=topic_id]:checked').val())<3){
				...
				var house = $("[name=house]"),
                    copyright = $("[name=copyright]:checked").val(),
	...
	if($(obj).val()=="3"){
		if($(obj).is(":checked")){
			...
			$(".deployed").each(function(){
				if($(this).has("[name=topic_id], [name=rent_type], [name=live_point], [name=orientir], [name=residents], [name=deliv_period], [name=ap_view_date], [name=ap_race_date], [data-id=furn_list], [name=price], [type=button], [type=submit], [name=type_id], [name=room_count]").length == 0){
		...
		if($(obj).is(":checked")){
			...
			$(".deployed").each(function(){
				if($(this).has("[name=topic_id], [name=rent_type], [name=live_point], [name=orientir], [name=residents], [name=deliv_period], [name=ap_view_date], [name=ap_race_date], [data-id=furn_list], [name=price], [type=button], [type=submit]").length == 0){
...
<div class="info">	
	<div class="row">
		<div class="col-xs-12">
			...
			<legend><?=$title?> (<?echo $topic." - ".$parent;?>)
		...
		<?
		//print_r ($data_res);
		include "application/includes/filters_to_create/topic.php";		
		if($topic == "Аренда"){
			/*длительно/сутки*/
			//include "application/includes/filters_to_create/rent_type.php";
			include "application/includes/filters_to_create/deliv_period.php";
		}?>			
	</div>
	<div class="row">
		<div class="col-xs-12">
...
/*
	if($_SESSION['user'] == '3619'){
		echo "
			<div id='map' style='width: 50%; height: 200px;'>

			</div>";
		echo "<input type='hidden' data-id='coords' value='".$str."'><br/><br/><br/><br/><br/><br/>";
		unset($mc, $str, $num);
	}
	/**/
		?>		
	...
	<div class="row">
		...
		if($parent == "Квартиры"){
			...
			if($topic == "Аренда"){
				include "application/includes/filters_to_create/residents.php";	
				//include "application/includes/filters_to_create/deliv_period.php";	
			}			
			...
			if($topic == "Аренда"){
				include "application/includes/filters_to_create/ap_view_date.php";	
				include "application/includes/filters_to_create/ap_race_date.php";
			}
	...
	<div class="row">
		<div class="col-xs-6">		
			...
			<p style = "color:red;"><i> В поле "<b>цена</b>" пишется полная стоимость со всеми коммунальными.<br/>
			Исключение вода и свет для них есть отдельное поле!</i></p>

			<?php if($topic == "Аренда"){ 				
				include "application/includes/filters_to_create/price.php";
				include "application/includes/filters_to_create/prepayment.php";
				include "application/includes/filters_to_create/torg.php";
				if($parent != "Гаражи" && $parent != "Земля"){
					include "application/includes/filters_to_create/utility_payment.php";
				}
				if($parent != "Земля"){
					include "application/includes/filters_to_create/deposit.php";
				}
				include "application/includes/filters_to_create/commission.php";
//				include "application/includes/filters_to_create/ap_view_price.php";
			}else{
				include "application/includes/filters_to_create/price.php";
			...
			if (isset($photos[0]['photo'])){
				...
				for($i=0; $i<count($photos); $i++){
					...
					if(file_exists($way))
						...
						<div class='image' data-photo_id='".$photos[$i]['id']."' data-way='{$way}' style='background-image:url({$way});background-size:cover; text-align: center; display: inline-block;margin: 5px 5px 5px 0;'></div><span class='glyphicon glyphicon-remove right' style='margin-left: -25px;background-color: #fff; border-radius: 15px; padding: 5px;cursor: pointer; color: #d9534f;' onClick='DeletePhotoByWay(\"{$way}\")'>
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<?if($topic == "Аренда" && (!isset($_GET['active']) || $_GET['active']==1)){?>
				...
				<div class="col-xs-3 deployed" style="max-width: 220px !important;">
					<input type="button" class="btn btn-primary right" onClick="$('[name=group]').val('1'); $('#sendAll').click();" value="Отправить своей группе" />
				</div>
				<div class="col-xs-2 deployed">
					<input type="submit" id='sendAll' class="btn btn-success right" name="submit" value="Отправить всем" />
				</div>
