...
if($_SESSION['mobile']==1){ ?><div class="col-xs-1 deployed" style = 'width:99%;'><?}?>
		...
		<div class="col-xs-1 deployed" style = '<?=$_SESSION['mobile']==1?'width: 5%;margin-right:2%; min-width: 60px !important':''?>'>
			<label class="signature">Мебель</label>	
			<select name="furn" class="form-control" style = '<?=$_SESSION['mobile']==1?'width: auto;':''?>'>
				<option value="">меб</option>
                <option value="1" <?=Helper::FilterVal('furn') == 1 ? "selected":''?>>есть</option>
                <option value="0" <?=Helper::FilterVal('furn') === '0' ? "selected":''?>>нет</option>
			</select>
		</div>
		<div class="col-xs-1 deployed" style = '<?=$_SESSION['mobile']==1?'width: 5%;min-width: 50px !important':''?>'>
			<label class="signature">Хол.</label>
			<select name="refrig" class="form-control" style = '<?=$_SESSION['mobile']==1?'width: auto;':''?>'>
		...
		<?if($data[0]['user_id'] != 'ngs' && $_GET['action']!="favorites_parse"){?>
			<div class="col-xs-1 deployed" style = '<?=$_SESSION['mobile']==1?'width: 5%;min-width: 50px !important':''?>'>
				...
				<select name="tv" class="form-control" style = '<?=$_SESSION['mobile']==1?'width: auto;':''?>'>
			...
			<div class="col-xs-1 deployed" style = '<?=$_SESSION['mobile']==1?'width: 5%;min-width: 50px !important':''?>'>
				...
				<select name="washing" class="form-control" style = '<?=$_SESSION['mobile']==1?'width: auto;':''?>'>
