<?if($_SESSION['search_user_id'] != "ngs" && $_GET['action']!="pay_parse" && $_GET['action']!="parse"){?>		
	<div class="col-xs-2 deployed">	
		<input type="text" class="form-control" data-name="an-list" placeholder="агентство"
                value="<?if(Helper::FilterVal("company_id")!="") echo $data[0]['company_name'];?>">
		<div class="an_list" style="display: none;overflow: auto; height: 250px;"></div>
		<input type="hidden" name="company_id"
               value="<?=(Helper::FilterVal("company_id")) ? Helper::FilterVal("company_id") : ''?>">
	</div>
	...
	if(Helper::FilterVal('task') != 'profile') {  ?>
        <div class="btn-group medium" data-toggle="buttons">
            ...
            <label onClick="window.location='/?task=main&action=pay_parse&parent_id=<?=$parent_id?>&topic_id=2'"
                ...
                <input type="radio" <?=(Helper::FilterVal('action') == "pay_parse") ?  "checked" : ''?>>частники
            </label>
        </div>
